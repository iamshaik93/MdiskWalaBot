...
async def validate_q(q):
    ...
    query = re.sub(r"z+|((send|snd|giv(e)?|gib)(\sme)?)|new|hd|-|the|in|\(|\)|dedo|print|full|bhai|latest|br((o|u)h?)*um(o)*|aya(k)*(um(o)*)?|any(one)|with\ssubtitle(s)?)", "", query.lower(), flags=re.IGNORECASE)
    return query.strip()
